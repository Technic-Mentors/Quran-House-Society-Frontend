...
export default function AudioLectureDetail() {
    const { title } = useParams()
    const [lectureDetail, setLectureDetail] = useState([])
    const formattedTitle = title.replace(/-/g, ' ')
    const LectureDetail = async () => {
        const res = await fetch(`https://quran-house-society-backend.vercel.app/api/getAudioLecture/${formattedTitle}`, {
            method: "GET"
        })
            ...
            <div className="container py-4">
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <iframe width="100%" height="500px" src={lectureDetail.audio && lectureDetail.audio.startsWith("https://www.youtube.com/embed") ? lectureDetail.audio : getEmbeddableUrl(lectureDetail.audio)} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
                    </div>
                </div>
